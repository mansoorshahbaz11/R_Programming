...
y<-"Unemployed"
z<-"Self-Employed"
emp_status<-factor(c(x,y,z,x,z,y,x,z,y,))
emp_status
```
...
```{r }
customer_bank = data.frame(CustomerID <- c("Alicia","Nancy","Fernando","Louis","Diane"), hasAccount <- c(F,T,T,T,NA), totalBalance <-c(NA,10000,100,0,NA))
```
...
```{r }
for (r in 1:nrow(mat_1)) {
  for (c in 1:ncol(mat_1)) {
    print(mat_1[r,c])
...
```{r }
apply(numbers_1,2,mean)
apply(numbers_1, 2, sd)
```
